...
import { ChannelEnum, User, Company } from '../company-data/company-data.interface';
...
describe('NotificationsService', () => {
  ...
  const mockedCompanyDataService = {
    getCompanyById: jest.fn().mockReturnValue(
      generateCompany('1', 'FirstCompany', [], [])
    )
  }
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [NotificationService, CompanyDataService, NotificationDataService],
    })
